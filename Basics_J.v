...
Proof.
  intros x y.   (* move both quantifiers into the context *)
...
Proof.
  ...
  (* Case "n = 0". *)
    reflexivity.
  (* Case "n = S n'".*)
    simpl. rewrite -> plus_n_Sm. rewrite -> plus_comm. simpl. rewrite <- IHn'. reflexivity.
Qed.
...
(** 紙を何枚か用意して、下に続く定理が(a)簡約と書き換えだけで証明可能か、(b)[destruct]を使ったcase分割が必要になるか、(c)帰納法が必要となるか、あ属すかを、紙の上だけで考えなさい。予測を紙に書いたら、実際に証明を完成させなさい。証明にはCoqを用いてかまいません。最初に紙を使ったのは「初心忘れるべからず」といった理由です。 *)
